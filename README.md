...
1. **고객 접수** (`/`) — 단지명(주소검색, 필수)/동/호수/연락처/카테고리/상세내용/사진(최대 10장) 입력 → 제출 즉시 무작위 6자리 접수번호 발급
2. **접수완료 팝업 → 자동 이동** — 접수 직후 팝업으로 접수번호 안내, 약 2초 후 진행상태 확인 화면으로 자동 이동
3. **진행상태 확인** (`/status`) — 접수번호 + 연락처 뒷 4자리로 조회합니다. 5회 실패하면 사람 확인(CAPTCHA), 10회 실패하면 15분 재시도 제한이 적용됩니다.
...
담당자 접속코드와 Flask 세션 키는 환경변수로 **반드시** 지정해야 합니다. 둘 중 하나라도 빠졌거나 이전 기본값(`theraum2026`)이면 앱은 시작하지 않습니다.

```bash
export MANAGER_PASSCODE="원하는코드"
export SECRET_KEY="충분히 긴 무작위 문자열"
# 로컬 HTTP 테스트 시에만 필요합니다. HTTPS 운영 환경에서는 설정하지 마세요.
